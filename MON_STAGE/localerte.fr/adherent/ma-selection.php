...
<head>
<?php $head_title='Ma sélection - Localerte.fr'; include_once(__DIR__.'/inc/head.php');?>
		...
		for($i=0;$i<sizeof($liste->occurrence);$i++)
		{
			...
			('
				<div class="annonce" id="annonce_'.$liste->occurrence[$i]['identifiant'].'">
				  ...
				  <div class="colonne2">
					'.($liste->occurrence[$i]['image'] && strpos($liste->occurrence[$i]['image'],'193.164')===false?'<img src="'.ma_htmlentities(HTTP_STATIC.'/api/gd/'.$liste->occurrence[$i]['identifiant'].'.png').'" alt="" width="160" height="120">':'<img src="'.ma_htmlentities(HTTP_STATIC.'/img/indisponible.png').'" alt="" width="160" height="120">').'
				  </div>
				  <div class="colonne3">
