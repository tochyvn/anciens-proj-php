...
(function($){
	...
	$.refresh=function(){
		...
		$('.resultat').each(function(){$(this).ajaxReplace({url:$(this).attr('href'),'find':'#resultat','event':'click',html:'...: Attente de la r&eacute;ponse :...'}).attr('href','javascript:void(0);');});
		$('.pre-annonce').annonce();
		
		$map=$('#map[title]');
		if($map.length){
			conf=JSON.parse($map.attr('title'));
			...
			for(i=0;i<conf['marker'].length;i++)
			{
				$map.osmLeaflet('addMarker',[{
					latitude  : conf['marker'][i]['latitude'],
					longitude : conf['marker'][i]['longitude'],
					'click'   : conf['marker'][i]['click'],
					'icon'    : {
						iconUrl:conf['iconUrl'],
						iconSize:[conf['iconSize'][0],conf['iconSize'][1]]
					}
				}]);
			}
		}
...
$(window).load(function(){
	...
	//
	resultat=window.location.href.match(/msgbox_query=([^&#]+)/)
	...
	if(typeof msgbox_message_special_01=='string' && !$.storage.existSession('msgbox_message_special_01')) setTimeout(function(){$.msgBox({classCSS:'msgbox msgbox-message',html:msgbox_message_special_01,'close':function(){},'success':function(){}}); $.storage.setSession('msgbox_message_special_01',true);},10000);
