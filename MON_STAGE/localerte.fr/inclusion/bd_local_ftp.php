<?php
	...
	define('DB_LOCAL_SERVEUR','aicom.co.fr.clara.net');
	define('DB_LOCAL_UTILISATEUR','aicom');
	define('DB_LOCAL_PASSE','Bah8Aesh');
	define('DB_LOCAL_CHEMIN_DISTANT','/base_de_donnees/localerte/');
	define('DB_LOCAL_CHEMIN_LOCAL','D:\\Sites\\v2.localerte.com\\base_de_donnees\\');
...
<?php
	...
	$ftp=ftp_connect(DB_LOCAL_SERVEUR); 
	if(ftp_login($ftp,DB_LOCAL_UTILISATEUR,DB_LOCAL_PASSE))
	{
		
		if(ftp_get($ftp,DB_LOCAL_CHEMIN_LOCAL.DB_LOCAL_BASE.'.sql',DB_LOCAL_CHEMIN_DISTANT.DB_LOCAL_BASE.'.sql',FTP_BINARY))
		{
			...
			flush();
			$fichier=ftp_nlist($ftp,DB_LOCAL_CHEMIN_DISTANT);
			
			for($i=0;$i<sizeof($fichier);$i++)
			{
				if(preg_match('/'.DB_LOCAL_EXTENSION.'$/',$fichier[$i]))
				{
					$fichier[$i]=basename($fichier[$i]);
					print('Fichier : <b>'.$fichier[$i].'</b><br />'.CRLF);
					flush();
					print(DB_LOCAL_DENTATION.'T&eacute;l&eacute;chargement du fichier<br />'.CRLF);
					flush();
					if(ftp_get($ftp,DB_LOCAL_CHEMIN_LOCAL.$fichier[$i],DB_LOCAL_CHEMIN_DISTANT.$fichier[$i],FTP_BINARY))
					{
					...
					{
						print(DB_LOCAL_DENTATION.'<span style="color: FF0000;">Erreur du t&eacute;l&eacute;chargement</span><br />'.CRLF);
						flush();
					}
				}
			}
