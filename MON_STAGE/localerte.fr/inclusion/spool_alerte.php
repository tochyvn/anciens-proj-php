<?php
	...
	class ld_spool_alerte extends ld_sql
	{
		private /*var*/ $champs;
		...
		public function lire()
		{
			...
			('
				select
					unix_timestamp(spool_alerte_chargement) as spool_alerte_chargement,
					unix_timestamp(spool_alerte_debut) as spool_alerte_debut,
					unix_timestamp(spool_alerte_fin) as spool_alerte_fin,
					spool_alerte_total,
					(select count(adherent) from spool_alerte) as spool_alerte_restant,
					spool_alerte_envoye
				from preference
			');
			...
			{
				$this->champs['chargement']=$occurrence['spool_alerte_chargement'];
				$this->champs['debut']=$occurrence['spool_alerte_debut'];
				$this->champs['fin']=$occurrence['spool_alerte_fin'];
				$this->champs['total']=$occurrence['spool_alerte_total'];
				if($this->champs['chargement']===NULL)
					$this->champs['restant']=NULL;
				else
					$this->champs['restant']=$occurrence['spool_alerte_restant'];
				$this->champs['envoye']=$occurrence['spool_alerte_envoye'];
			}
		...
		private function modifier()
		{
			...
			('
				update preference
				set
					spool_alerte_chargement='.(($this->champs['chargement']!==NULL)?('\''.addslashes(date(SQL_DATETIME,$this->champs['chargement'])).'\''):('null')).',
					spool_alerte_debut='.(($this->champs['debut']!==NULL)?('\''.addslashes(date(SQL_DATETIME,$this->champs['debut'])).'\''):('null')).',
					spool_alerte_fin='.(($this->champs['fin']!==NULL)?('\''.addslashes(date(SQL_DATETIME,$this->champs['fin'])).'\''):('null')).',
					spool_alerte_total='.(($this->champs['total']!==NULL)?('\''.addslashes($this->champs['total']).'\''):('null')).',
					spool_alerte_envoye='.(($this->champs['envoye']!==NULL)?('\''.addslashes($this->champs['envoye']).'\''):('null')).'
			');
		}
		...
		public function charger()
		{
			if($this->champs['chargement']===NULL)
			{
				...
				$this->executer
				('
					insert into spool_alerte (adherent)
		...
		public function vider()
		{
			if($this->champs['chargement']!==NULL)
			{
				$this->executer('truncate table spool_alerte');				
				$this->champs['chargement']=NULL;
				$this->champs['debut']=NULL;
				$this->champs['fin']=NULL;
				$this->champs['total']=NULL;
			...
			if($this->champs['chargement']!==NULL && $this->champs['envoye']=='NON')
			{
				...
				do
				{
					...
					('
						...
						from spool_alerte
							inner join adherent on spool_alerte.adherent=adherent.identifiant
					...
					for($i=0;$i<sizeof($spool);$i++)
					{
						...
						$this->executer('delete from spool_alerte where adherent=\''.addslashes($spool[$i]['adherent']).'\'');
