...
abstract class Utilisateur implements ActiveRecordInterface
{
    ...
    public function setPseudo($v)
    {
        ...
        if ($this->pseudo !== $v) {
            $this->pseudo = $v;
            $this->modifiedColumns[UtilisateurTableMap::COL_PSEUDO] = true;
        }
    ...
    public function setRole($v)
    {
        ...
        if ($this->role !== $v) {
            $this->role = $v;
            $this->modifiedColumns[UtilisateurTableMap::COL_ROLE] = true;
        }
    ...
    public function hydrate($row, $startcol = 0, $rehydrate = false, $indexType = TableMap::TYPE_NUM)
    {
        try {
            ...
            $col = $row[TableMap::TYPE_NUM == $indexType ? 0 + $startcol : UtilisateurTableMap::translateFieldName('Email', TableMap::TYPE_PHPNAME, $indexType)];
            $this->email = (null !== $col) ? (string) $col : null;

            $col = $row[TableMap::TYPE_NUM == $indexType ? 1 + $startcol : UtilisateurTableMap::translateFieldName('Password', TableMap::TYPE_PHPNAME, $indexType)];
            $this->password = (null !== $col) ? (string) $col : null;

            $col = $row[TableMap::TYPE_NUM == $indexType ? 2 + $startcol : UtilisateurTableMap::translateFieldName('Pseudo', TableMap::TYPE_PHPNAME, $indexType)];
            $this->pseudo = (null !== $col) ? (int) $col : null;

            $col = $row[TableMap::TYPE_NUM == $indexType ? 3 + $startcol : UtilisateurTableMap::translateFieldName('Role', TableMap::TYPE_PHPNAME, $indexType)];
    ...
    protected function doInsert(ConnectionInterface $con)
    {
        ...
        if ($this->isColumnModified(UtilisateurTableMap::COL_PSEUDO)) {
            $modifiedColumns[':p' . $index++]  = 'pseudo';
        }
        if ($this->isColumnModified(UtilisateurTableMap::COL_ROLE)) {
            ...
            foreach ($modifiedColumns as $identifier => $columnName) {
                switch ($columnName) {
                    ...
                    case 'password':
                        ...
                        break;
                    case 'pseudo':
                        $stmt->bindValue($identifier, $this->pseudo, PDO::PARAM_INT);
                        break;
                    case 'role':
    ...
    public function buildCriteria()
    {
        ...
        if ($this->isColumnModified(UtilisateurTableMap::COL_PSEUDO)) {
            $criteria->add(UtilisateurTableMap::COL_PSEUDO, $this->pseudo);
        }
        if ($this->isColumnModified(UtilisateurTableMap::COL_ROLE)) {
            $criteria->add(UtilisateurTableMap::COL_ROLE, $this->role);
        }
    ...
    public function setAchats(Collection $achats, ConnectionInterface $con = null)
    {
        ...
        foreach ($achatsToDelete as $achatRemoved) {
            $achatRemoved->setUtilisateur(null);
        }
     ...
     * @return ObjectCollection|ChildAchat[] List of ChildAchat objects
     */
    public function getAchatsJoinJetons(Criteria $criteria = null, ConnectionInterface $con = null, $joinBehavior = Criteria::LEFT_JOIN)
    {
        $query = ChildAchatQuery::create(null, $criteria);
        $query->joinWith('Jetons', $joinBehavior);
    ...
    public function setMises(Collection $mises, ConnectionInterface $con = null)
    {
        ...
        foreach ($misesToDelete as $miseRemoved) {
            $miseRemoved->setUtilisateur(null);
        }
