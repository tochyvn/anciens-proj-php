...
/**
 * Base class that represents a row from the 'Jetons' table.
 *
 *
 *
* @package    propel.generator..Base
*/
abstract class Jetons implements ActiveRecordInterface
{
    /**
     * TableMap class name
     */
    const TABLE_MAP = '\\Map\\JetonsTableMap';
    ...
    /**
     * Initializes internal state of Base\Jetons object.
     */
    ...
    /**
     * Compares this with another <code>Jetons</code> instance.  If
     * <code>obj</code> is an instance of <code>Jetons</code>, delegates to
     * <code>equals(Jetons)</code>.  Otherwise, returns <code>false</code>.
     *
     ...
     *
     * @return $this|Jetons The current object, for fluid interface
     */
     ...
     * @return $this|\Jetons The current object (for fluent API support)
     */
    public function setId($v)
    {
        ...
        if ($this->id !== $v) {
            $this->id = $v;
            $this->modifiedColumns[JetonsTableMap::COL_ID] = true;
        }
    ...
    public function setDesignation($v)
    {
        ...
        if ($this->designation !== $v) {
            $this->designation = $v;
            $this->modifiedColumns[JetonsTableMap::COL_DESIGNATION] = true;
        }
    ...
    public function hydrate($row, $startcol = 0, $rehydrate = false, $indexType = TableMap::TYPE_NUM)
    {
        try {
            ...
            $col = $row[TableMap::TYPE_NUM == $indexType ? 0 + $startcol : JetonsTableMap::translateFieldName('Id', TableMap::TYPE_PHPNAME, $indexType)];
            $this->id = (null !== $col) ? (int) $col : null;

            $col = $row[TableMap::TYPE_NUM == $indexType ? 1 + $startcol : JetonsTableMap::translateFieldName('Designation', TableMap::TYPE_PHPNAME, $indexType)];
            ...
            return $startcol + 2; // 2 = JetonsTableMap::NUM_HYDRATE_COLUMNS.

        } catch (Exception $e) {
            throw new PropelException(sprintf('Error populating %s object', '\\Jetons'), 0, $e);
        }
    }
    ...
    public function reload($deep = false, ConnectionInterface $con = null)
    {
        ...
        $dataFetcher = ChildJetonsQuery::create(null, $this->buildPkeyCriteria())->setFormatter(ModelCriteria::FORMAT_STATEMENT)->find($con);
     ...
     * @return void
     * @throws PropelException
     * @see Jetons::setDeleted()
     * @see Jetons::isDeleted()
     */
    public function delete(ConnectionInterface $con = null)
    {
        ...
        $con->transaction(function () use ($con) {
            $deleteQuery = ChildJetonsQuery::create()
    ...
    public function save(ConnectionInterface $con = null)
    {
        ...
        return $con->transaction(function () use ($con) {
            ...
            if ($ret) {
                ...
                $this->postSave($con);
                JetonsTableMap::addInstanceToPool($this);
            } else {
                $affectedRows = 0;
            }
        ...
        if (!$this->alreadyInSave) {
            ...
            if ($this->achatsScheduledForDeletion !== null) {
                if (!$this->achatsScheduledForDeletion->isEmpty()) {
                    foreach ($this->achatsScheduledForDeletion as $achat) {
    ...
    protected function doInsert(ConnectionInterface $con)
    {
        ...
        if ($this->isColumnModified(JetonsTableMap::COL_ID)) {
            $modifiedColumns[':p' . $index++]  = 'id';
        }
        if ($this->isColumnModified(JetonsTableMap::COL_DESIGNATION)) {
            $modifiedColumns[':p' . $index++]  = 'designation';
        }

        $sql = sprintf(
            'INSERT INTO Jetons (%s) VALUES (%s)',
    ...
    {
        $pos = JetonsTableMap::translateFieldName($name, $type, TableMap::TYPE_NUM);
    ...
    public function toArray($keyType = TableMap::TYPE_PHPNAME, $includeLazyLoadColumns = true, $alreadyDumpedObjects = array(), $includeForeignObjects = false)
    {
        ...
        if (isset($alreadyDumpedObjects['Jetons'][$this->hashCode()])) {
            return '*RECURSION*';
        }
        $alreadyDumpedObjects['Jetons'][$this->hashCode()] = true;
        $keys = JetonsTableMap::getFieldNames($keyType);
        $result = array(
            $keys[0] => $this->getId(),
    ...
    {
        $criteria = new Criteria(JetonsTableMap::DATABASE_NAME);

        if ($this->isColumnModified(JetonsTableMap::COL_ID)) {
            $criteria->add(JetonsTableMap::COL_ID, $this->id);
        }
        if ($this->isColumnModified(JetonsTableMap::COL_DESIGNATION)) {
            $criteria->add(JetonsTableMap::COL_DESIGNATION, $this->designation);
        }
    ...
    {
        $criteria = ChildJetonsQuery::create();
        $criteria->add(JetonsTableMap::COL_ID, $this->id);
     ...
     * objects.
     *
     * @param      object $copyObj An object of \Jetons (or compatible) type.
     ...
     *
     * @param  boolean $deepCopy Whether to also copy all rows that refer (by fkey) to the current row.
     * @return \Jetons Clone of current object.
     * @throws PropelException
     */
     ...
     * If this ChildJetons is new, it will return
     ...
     * @param      ConnectionInterface $con Optional connection object
     * @return $this|ChildJetons The current object (for fluent API support)
     */
    public function setAchats(Collection $achats, ConnectionInterface $con = null)
    {
        ...
        $this->achatsScheduledForDeletion = $achatsToDelete;

        foreach ($achatsToDelete as $achatRemoved) {
            $achatRemoved->setJetons(null);
        }
    ...
    public function removeAchat(ChildAchat $achat)
    {
        if ($this->getAchats()->contains($achat)) {
            ...
            $this->achatsScheduledForDeletion[]= $achat;
            $achat->setJetons(null);
        }
     ...
     * Otherwise if this Jetons is new, it will return
     * an empty collection; or if this Jetons has previously
     * been saved, it will retrieve related Achats from storage.
     *
     * This method is protected by default in order to keep the public
     * api reasonable.  You can provide public methods for those you
     * actually need in Jetons.
     *
     * @param      Criteria $criteria optional Criteria object to narrow the query
     * @param      ConnectionInterface $con optional connection object
     ...
     * @return ObjectCollection|ChildAchat[] List of ChildAchat objects
     */
    public function getAchatsJoinUtilisateur(Criteria $criteria = null, ConnectionInterface $con = null, $joinBehavior = Criteria::LEFT_JOIN)
    {
        $query = ChildAchatQuery::create(null, $criteria);
    ...
    {
        return (string) $this->exportTo(JetonsTableMap::DEFAULT_STRING_FORMAT);
    }
