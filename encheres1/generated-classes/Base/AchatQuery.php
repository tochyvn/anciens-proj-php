...
abstract class AchatQuery extends ModelCriteria
{
     ...
     */
    public function __construct($dbName = 'encheres', $modelName = '\\Achat', $modelAlias = null)
    {
     ...
     * </code>
     *
     * @param array[$date_achat, $pseudo, $id_pack] $key Primary key to use for the query
    ...
    public function findPk($key, ConnectionInterface $con = null)
    {
        ...
        if ((null !== ($obj = AchatTableMap::getInstanceFromPool(serialize(array((string) $key[0], (string) $key[1], (string) $key[2]))))) && !$this->formatter) {
    ...
    {
        $sql = 'SELECT date_achat, pseudo, id_pack FROM ACHAT WHERE date_achat = :p0 AND pseudo = :p1 AND id_pack = :p2';
        try {
        ...
        if ($row = $stmt->fetch(\PDO::FETCH_NUM)) {
            ...
            $obj = new ChildAchat();
            $obj->hydrate($row);
            AchatTableMap::addInstanceToPool($obj, serialize(array((string) $key[0], (string) $key[1], (string) $key[2])));
        }
        $stmt->closeCursor();
    ...
    public function filterByPrimaryKey($key)
    {
        ...
        $this->addUsingAlias(AchatTableMap::COL_ID_PACK, $key[2], Criteria::EQUAL);
    ...
    public function filterByPrimaryKeys($keys)
    {
        ...
        foreach ($keys as $key) {
            ...
            $cton0->addAnd($cton1);
            $cton2 = $this->getNewCriterion(AchatTableMap::COL_ID_PACK, $key[2], Criteria::EQUAL);
            $cton0->addAnd($cton2);
            $this->addOr($cton0);
        }
     ...
     * Example usage:
     * <code>
     * $query->filterByDateAchat('fooValue');   // WHERE date_achat = 'fooValue'
     * $query->filterByDateAchat('%fooValue%'); // WHERE date_achat LIKE '%fooValue%'
     * </code>
     *
     * @param     string $dateAchat The value to use as filter.
    ...
    public function filterByDateAchat($dateAchat = null, $comparison = null)
    {
        if (null === $comparison) {
            ...
            } elseif (preg_match('/[\%\*]/', $dateAchat)) {
                $dateAchat = str_replace('*', '%', $dateAchat);
    ...
    public function filterByUser($user, $comparison = null)
    {
        if ($user instanceof \User) {
            ...
            return $this
                ->addUsingAlias(AchatTableMap::COL_PSEUDO, $user->toKeyValue('PrimaryKey', 'Pseudo'), $comparison);
        } else {
    ...
    public function prune($achat = null)
    {
        if ($achat) {
            ...
            $this->addCond('pruneCond2', $this->getAliasedColName(AchatTableMap::COL_ID_PACK), $achat->getIdPack(), Criteria::NOT_EQUAL);
