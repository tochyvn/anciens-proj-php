 ...
 *
 * @method     ChildMise findOneByPrix(string $prix) Return the first ChildMise filtered by the prix column
 ...
 *
 * @method     ChildMise requireOneByPrix(string $prix) Return the first ChildMise filtered by the prix column and throws \Propel\Runtime\Exception\EntityNotFoundException when not found
 ...
 * @method     ChildMise[]|ObjectCollection findByPrix(string $prix) Return ChildMise objects filtered by the prix column
...
abstract class MiseQuery extends ModelCriteria
{
     ...
     * </code>
     *
     * @param array[$prix, $pseudo, $id_ench] $key Primary key to use for the query
    ...
    public function findPk($key, ConnectionInterface $con = null)
    {
        ...
        if ((null !== ($obj = MiseTableMap::getInstanceFromPool(serialize(array((string) $key[0], (string) $key[1], (string) $key[2]))))) && !$this->formatter) {
    ...
    {
        $sql = 'SELECT prix, pseudo, id_ench FROM MISE WHERE prix = :p0 AND pseudo = :p1 AND id_ench = :p2';
        try {
        ...
        if ($row = $stmt->fetch(\PDO::FETCH_NUM)) {
            ...
            $obj = new ChildMise();
            $obj->hydrate($row);
            MiseTableMap::addInstanceToPool($obj, serialize(array((string) $key[0], (string) $key[1], (string) $key[2])));
        }
        $stmt->closeCursor();
    ...
    {
        $this->addUsingAlias(MiseTableMap::COL_PRIX, $key[0], Criteria::EQUAL);
        $this->addUsingAlias(MiseTableMap::COL_PSEUDO, $key[1], Criteria::EQUAL);
        $this->addUsingAlias(MiseTableMap::COL_ID_ENCH, $key[2], Criteria::EQUAL);
    ...
    public function filterByPrimaryKeys($keys)
    {
        ...
        foreach ($keys as $key) {
            $cton0 = $this->getNewCriterion(MiseTableMap::COL_PRIX, $key[0], Criteria::EQUAL);
            $cton1 = $this->getNewCriterion(MiseTableMap::COL_PSEUDO, $key[1], Criteria::EQUAL);
            $cton0->addAnd($cton1);
            $cton2 = $this->getNewCriterion(MiseTableMap::COL_ID_ENCH, $key[2], Criteria::EQUAL);
            $cton0->addAnd($cton2);
            $this->addOr($cton0);
        }
    ...
    public function filterByUser($user, $comparison = null)
    {
        if ($user instanceof \User) {
            ...
            return $this
                ->addUsingAlias(MiseTableMap::COL_PSEUDO, $user->toKeyValue('PrimaryKey', 'Pseudo'), $comparison);
        } else {
    ...
    {
        if ($mise) {
            $this->addCond('pruneCond0', $this->getAliasedColName(MiseTableMap::COL_PRIX), $mise->getPrix(), Criteria::NOT_EQUAL);
            $this->addCond('pruneCond1', $this->getAliasedColName(MiseTableMap::COL_PSEUDO), $mise->getPseudo(), Criteria::NOT_EQUAL);
            $this->addCond('pruneCond2', $this->getAliasedColName(MiseTableMap::COL_ID_ENCH), $mise->getIdEnch(), Criteria::NOT_EQUAL);
