 ...
 * @method     ChildUserQuery orderByDatenaiss($order = Criteria::ASC) Order by the dateNaiss column
 ...
 * @method     ChildUserQuery groupByDatenaiss() Group by the dateNaiss column
 ...
 * @method     ChildUser findOneByDatenaiss(string $dateNaiss) Return the first ChildUser filtered by the dateNaiss column
 ...
 * @method     ChildUser requireOneByDatenaiss(string $dateNaiss) Return the first ChildUser filtered by the dateNaiss column and throws \Propel\Runtime\Exception\EntityNotFoundException when not found
 ...
 * @method     ChildUser[]|ObjectCollection findByDatenaiss(string $dateNaiss) Return ChildUser objects filtered by the dateNaiss column
...
abstract class UserQuery extends ModelCriteria
{
     ...
     */
    public function __construct($dbName = 'SITE_ENCHERES', $modelName = '\\User', $modelAlias = null)
    {
     ...
     * </code>
     *
     * @param array[$pseudo, $passwd] $key Primary key to use for the query
    ...
    {
        $sql = 'SELECT pseudo, passwd, nom, prenom, dateNaiss, pays, adresse, telephone, email, role FROM USER WHERE pseudo = :p0 AND passwd = :p1';
        try {
    ...
    {
        $this->addUsingAlias(UserTableMap::COL_PSEUDO, $key[0], Criteria::EQUAL);
        $this->addUsingAlias(UserTableMap::COL_PASSWD, $key[1], Criteria::EQUAL);
    ...
    public function filterByPrimaryKeys($keys)
    {
        ...
        foreach ($keys as $key) {
            $cton0 = $this->getNewCriterion(UserTableMap::COL_PSEUDO, $key[0], Criteria::EQUAL);
            $cton1 = $this->getNewCriterion(UserTableMap::COL_PASSWD, $key[1], Criteria::EQUAL);
            $cton0->addAnd($cton1);
            $this->addOr($cton0);
        }
    ...
    /**
     * Filter the query on the dateNaiss column
     *
     * Example usage:
     * <code>
     * $query->filterByDatenaiss('2011-03-14'); // WHERE dateNaiss = '2011-03-14'
     * $query->filterByDatenaiss('now'); // WHERE dateNaiss = '2011-03-14'
     * $query->filterByDatenaiss(array('max' => 'yesterday')); // WHERE dateNaiss > '2011-03-13'
     * </code>
     *
    ...
    public function filterByDatenaiss($datenaiss = null, $comparison = null)
    {
        if (is_array($datenaiss)) {
            ...
            if (isset($datenaiss['min'])) {
                $this->addUsingAlias(UserTableMap::COL_DATENAISS, $datenaiss['min'], Criteria::GREATER_EQUAL);
                $useMinMax = true;
            }
            if (isset($datenaiss['max'])) {
                $this->addUsingAlias(UserTableMap::COL_DATENAISS, $datenaiss['max'], Criteria::LESS_EQUAL);
                $useMinMax = true;
            }
            if ($useMinMax) {
                return $this;
        ...
        return $this->addUsingAlias(UserTableMap::COL_DATENAISS, $datenaiss, $comparison);
    }
    ...
    public function filterByAchat($achat, $comparison = null)
    {
        if ($achat instanceof \Achat) {
            return $this
                ...
                ->addUsingAlias(UserTableMap::COL_PASSWD, $achat->getPasswd(), $comparison);
        } else {
            throw new PropelException('filterByAchat() only accepts arguments of type \Achat');
        }
    }
    ...
    public function filterByMise($mise, $comparison = null)
    {
        if ($mise instanceof \Mise) {
            return $this
                ...
                ->addUsingAlias(UserTableMap::COL_PASSWD, $mise->getPasswd(), $comparison);
        } else {
            throw new PropelException('filterByMise() only accepts arguments of type \Mise');
        }
    }
    ...
    {
        if ($user) {
            $this->addCond('pruneCond0', $this->getAliasedColName(UserTableMap::COL_PSEUDO), $user->getPseudo(), Criteria::NOT_EQUAL);
            $this->addCond('pruneCond1', $this->getAliasedColName(UserTableMap::COL_PASSWD), $user->getPasswd(), Criteria::NOT_EQUAL);
