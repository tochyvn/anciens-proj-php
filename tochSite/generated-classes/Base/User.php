...
abstract class User implements ActiveRecordInterface
{
    ...
    /**
     * The value for the datenaiss field.
     * @var        \DateTime
     */
    protected $datenaiss;
    ...
    /**
     * Get the [optionally formatted] temporal [datenaiss] column value.
     *
     *
    ...
    public function getDatenaiss($format = NULL)
    {
        if ($format === null) {
            ...
            return $this->datenaiss instanceof \DateTime ? $this->datenaiss->format($format) : null;
        }
    }
    ...
    /**
     * Sets the value of [datenaiss] column to a normalized version of the date/time value specified.
     *
    ...
    public function setDatenaiss($v)
    {
        ...
        if ($this->datenaiss !== null || $dt !== null) {
            if ($this->datenaiss === null || $dt === null || $dt->format("Y-m-d") !== $this->datenaiss->format("Y-m-d")) {
                $this->datenaiss = $dt === null ? null : clone $dt;
                $this->modifiedColumns[UserTableMap::COL_DATENAISS] = true;
            }
    ...
    public function hydrate($row, $startcol = 0, $rehydrate = false, $indexType = TableMap::TYPE_NUM)
    {
        try {
            ...
            $this->datenaiss = (null !== $col) ? PropelDateTime::newInstance($col, null, 'DateTime') : null;
    ...
    protected function doInsert(ConnectionInterface $con)
    {
        ...
        if ($this->isColumnModified(UserTableMap::COL_DATENAISS)) {
            $modifiedColumns[':p' . $index++]  = 'dateNaiss';
        }
            ...
            foreach ($modifiedColumns as $identifier => $columnName) {
                switch ($columnName) {
                    ...
                    case 'prenom':
                        ...
                        break;
                    case 'dateNaiss':
                        $stmt->bindValue($identifier, $this->datenaiss ? $this->datenaiss->format("Y-m-d H:i:s") : null, PDO::PARAM_STR);
                        break;
                    case 'pays':
    ...
    public function buildCriteria()
    {
        ...
        if ($this->isColumnModified(UserTableMap::COL_DATENAISS)) {
            $criteria->add(UserTableMap::COL_DATENAISS, $this->datenaiss);
        }
        if ($this->isColumnModified(UserTableMap::COL_PAYS)) {
    ...
    {
        $pks = array();
        $pks[0] = $this->getPseudo();
        $pks[1] = $this->getPasswd();
    ...
    {
        $this->setPseudo($keys[0]);
        $this->setPasswd($keys[1]);
    }
    ...
    {
        return (null === $this->getPseudo()) && (null === $this->getPasswd());
    }
